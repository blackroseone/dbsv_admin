...
- **并行只读工具（v2.0.9）**：一次思考可输出多个工具调用（JSON 数组），只读工具线程池并行执行（最多 4 并发/次 5 个），多指标诊断提速；每个动作独立过 Harness 校验
- **上下文压缩（v2.0.9）**：对话历史过长时头尾保护 + 中间摘要，控制超长 prompt
- **迭代预算（v2.0.9）**：max_steps 配置化 + 重复动作死循环检测 + 历史字符预算，超限强制收敛
- **真实工具执行**：7 个工具均为真实实现
...
- **文档生成技能（v2.0.9）**：上传操作手册或从手册页直接「生成技能」，LLM 提炼为可复用技能（离线回退 + Curator 去重）
- **学习闭环（v2.0.8）**：成功诊断后自动沉淀技能（LLM 提炼/离线回退 + Curator 去重淘汰），诊断结论与 DBA 反馈自动写入长期记忆，下次诊断按关键词召回注入环境上下文
- **记忆语义召回（v2.0.9）**：记忆写入时自动编码向量，召回时语义检索 top-K（模型不可用回退关键词），并对主机/实例/集群记忆补图谱上下文
- **记忆事实校验（v2.0.9）**：写前与图谱实体/监控对象/知识库支撑交叉验证，无支撑结论跳过不写，防污染
- **DBA 反馈闭环（v2.0.9）**：结论后 👍/👎 + 纠正，反向修正该会话技能置信度/状态与记忆
- **变更类审批流（v2.0.10）**：运维操作分查询类（只读，不变）与变更类（修改参数/配置/变更命令）。变更类走通用流程：确认范围 → 创建操作计划 → DBA 审批 → 引擎按计划执行 → 遇错自分析自查询 → 追加新计划再审批，迭代直至完成。模型工具永远只读，写操作唯一通道 = 计划 + DBA 审批 + 引擎执行（Harness 变更白名单二次校验）。审批面板展示操作项临时列表 + 批准/拒绝。
- **会话范围化 + 多节点批量（v2.1.0）**：会话绑定拓扑资源池内的多节点范围（`scope_type`/`scope_json`），前端范围面板勾选节点（连接状态 ✅/⚠️ 一键补配）。引擎批量 fan-out：模型只写一次命令/SQL，逐节点用该节点 db_type 校验后并发执行，结果按节点表格呈现；批量变更 continue-on-error 收集失败节点并支持「仅重试失败节点」；引用范围外节点自动走「扩展操作范围」审批。手动技能 chips 注入完整操作指南。
...
#### v2.3.0 Agent 智能化增强
- **结论渲染可靠性**：多轮会话结论块唯一引用（不再串进旧块）；流式解析兼容推理模型 `reasoning_content`；LLM 双失败产出可见警告而非静默空转。
- **变更操作执行后强制验证**：系统提示规则 + 引擎拦截（批准执行后直接收敛会被拦一次强制验证）；命令超时感知（`timed_out/truncated` 返回字段 + 后台命令"请用 ps/tail 复查"引导）；输出截断「头+尾」保留成功/失败标记；计划操作超时 300s。
- **上下文与鲁棒性**：大结果只存摘要进 history（全量经 SSE 展示）；工具 JSON 解析失败自动回喂纠正；并行工具异常兜底；死循环检测指纹归一化；主循环墙钟超时；结论生成显式 `max_tokens`。

#### v2.3.0 操作范围树增强
- 节点树滚动修复（多节点池展开不再挤扁其它树，面板内滚动）；节点搜索过滤（池/服务器/实例/端口）；全部展开/折叠；折叠状态持久化；重渲染保留滚动位置与焦点。

#### v2.4.0 输入框悬浮化 + 模型选择
- 输入框悬浮化（agent + qa）：对话区底部居中、mask 渐淡、橙色 focus 高亮、发送按钮圆形↑、textarea 8 行自适应。
- agent 新增会话级模型选择（输入框左下方按钮 + 上弹列表）；qa 模型选择按钮化（移除顶部下拉框）。
- 历史记忆改 toggle 按钮；审批槽悬浮输入框上方、宽度跟随。
- 知识库检索补相邻块 + chunk 注入截断放宽（`KNOWLEDGE_CHUNK_INJECT_LIMIT`）；长期记忆召回阈值/上限配置化。
...
| overlap | 50 | 相邻块重叠字符数（仅字符边界切时生效） |
| 分块策略 | 句子边界优先 | 切块点优先落在句末标点（v2.5.0）；表格/代码退回字符边界切 |
...
> 分块 500 后阈值 0.55/0.60 → 0.75/0.80（实测 30 个真实问答 top-1 相似度 0.766~0.869，详见 version_update v2.0.5）。> v2.5.0 句界分块后阈值实测重校准（新分布见 version_update v2.5.0）。
...
当前版本：v2.5.3。完整更新记录见 `version_update.md`。
