...
- **会话范围化 + 多节点批量（v4.0.0）**：会话绑定拓扑资源池内的多节点范围（`scope_type`/`scope_json`），前端范围面板勾选节点（连接状态 ✅/⚠️ 一键补配）。引擎批量 fan-out：模型只写一次命令/SQL，逐节点用该节点 db_type 校验后并发执行，结果按节点表格呈现；批量变更 continue-on-error 收集失败节点并支持「仅重试失败节点」；引用范围外节点自动走「扩展操作范围」审批。手动技能 chips 注入完整操作指南。
...
| 分块策略 | 句子边界优先 | 切块点优先落在句末标点（v4.4.0）；表格/代码退回字符边界切 |
...
> 分块 500 后阈值 0.55/0.60 → 0.75/0.80（实测 30 个真实问答 top-1 相似度 0.766~0.869，详见 version_update v3.0.5）。> v4.4.0 句界分块后阈值实测重校准（新分布见 version_update v4.4.0）。
...
当前版本：v4.3.0。完整更新记录见 `version_update.md`。
