...
def search_entities_api():
    """搜索实体
    GET /api/kg/entities/search?q=MySQL&type=database_product&limit=20&neighbors=true&depth=1&max_relations=30
    """
    ...
    max_relations = request.args.get('max_relations', 30, type=int)
...
def get_entity_api(entity_id):
    ...
    filtered_rels = []
    per_type_limit = max(10, 100 // len(rel_groups)) if rel_groups else 100
    ...
    relationships = filtered_rels[:100]
...
def get_entity_neighbors_api(entity_id):
    """获取实体的邻居子图
    GET /api/kg/entities/123/neighbors?depth=2&relation_types=compatible_with,requires&max_relations=50
    """
    ...
    max_relations = request.args.get('max_relations', 50, type=int)
