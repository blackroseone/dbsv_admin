...
| `kb_files` | 知识库文件元数据 + 内容（**上传/重建时自动提取知识图谱**） | db_type, filename, file_path, file_size, content_text, tags |
...
**注意：** topo_clusters 表只保留核心字段，数据库类型、环境等信息通过 resource_pool_id 关联到 topo_resource_pools 表获取。

---

### 8. topo_servers — 物理机/节点表
...
### 11. topo_instance_relations — 实例关系表
...
| db_type | TEXT | 数据库类型（关联 sys_db_types 表） |
...
| 索引名 | 表名 | 字段 | 用途 |
|--------|------|------|------|
| idx_knowledge_files_db_type | kb_files | db_type | 按数据库类型查询 |
| idx_qa_history_db_type | qa_history | db_type | 按数据库类型查询 |
| idx_qa_history_created | qa_history | created_at DESC | 按时间排序 |
| idx_servers_cluster | topo_servers | cluster_id | 按集群查询 |
| idx_instances_server | topo_instances | server_id | 按节点查询 |
| idx_tenants_cluster | topo_tenants | cluster_id | 按集群查询 |
| idx_embeddings_file | kb_embeddings | file_id | 按文件查询 |
| idx_operation_logs_timestamp | audit_operation_logs | timestamp DESC | 按时间排序 |
| idx_operation_logs_module | audit_operation_logs | module | 按模块查询 |
...
| 表名 | 字段 | 变更时间 | 说明 |
|------|------|----------|------|
| kb_files | content_text | 2026-07-30 | 更新逻辑（`add_knowledge_file` 返回 file_id，支持知识图谱关联） |
| kb_files | file_path | 2026-07-30 | 新增 GaussDB 巡检文件（2个PDF，已提取496个实体） |
| topo_clusters | environment | 2026-07-07 | 移除（冗余字段，通过 resource_pool_id 关联获取） |
| topo_clusters | topology_type | 2026-07-07 | 移除（冗余字段） |
| topo_servers | cpu | 2026-07-07 | 新增 |
| topo_servers | memory | 2026-07-07 | 新增 |
| topo_servers | node_role | 2026-07-07 | 新增 |
| topo_servers | hardware_type | 2026-07-07 | 新增 |
| topo_servers | datacenter | 2026-07-07 | 新增 |
| topo_instances | role | 2026-07-07 | 新增 |
| topo_instances | tenant_id | 2026-07-07 | 新增 |
| topo_instances | tenant_role | 2026-07-07 | 新增 |
| topo_tenants | spec | 2026-07-07 | 新增 |
...
| 子表 | 字段 | 父表 | 约束 |
|------|------|------|------|
| topo_clusters | resource_pool_id | topo_resource_pools | ON DELETE CASCADE |
| topo_servers | resource_pool_id | topo_resource_pools | ON DELETE CASCADE |
| topo_instances | server_id | topo_servers | ON DELETE CASCADE |
| topo_instances | tenant_id | topo_tenants | ON DELETE SET NULL |
| topo_instance_relations | from_instance_id | topo_instances | ON DELETE CASCADE |
| topo_instance_relations | to_instance_id | topo_instances | ON DELETE CASCADE |
| kb_embeddings | file_id | kb_files | ON DELETE CASCADE |
| kg_relationships | from_entity_id | kg_entities | ON DELETE CASCADE |
| kg_relationships | to_entity_id | kg_entities | ON DELETE CASCADE |
| kg_chunk_entities | chunk_id | kb_embeddings | ON DELETE CASCADE |
| kg_chunk_entities | entity_id | kg_entities | ON DELETE CASCADE |
| topo_tenants | resource_pool_id | topo_resource_pools | ON DELETE CASCADE |
...
| 字段 | 类型 | 说明 |
|------|------|------|
| chunk_id | INTEGER | chunk ID（外键，关联 kb_embeddings 表） |
